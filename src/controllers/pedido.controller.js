...
const getPedidosPorCliente = (req, res) => {
    let desde = req.params.desde || 0;
    desde = Number(desde);
    let i = 0;
    let id = req.params.id;
...
const addPedido = (req, res) => {
    ...
    producto = Producto.findById(body.producto, (err, producto) => {
        if (err) {
            return res.status(400).json({
                ok: false,
...
module.exports = { getPedidos, getPedidoById, getPedidosPorCliente, addPedido, updatePedido, deletePedido };